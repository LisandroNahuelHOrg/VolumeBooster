...
softClipPair(x, y) = (
  x * (1.0 - softNorm) + wetL * softNorm,
  y * (1.0 - softNorm) + wetR * softNorm
)
with {
  drive = 1.0 + softNorm * 1.8;
  wetL = softSat(x * drive) / softSat(drive);
  wetR = softSat(y * drive) / softSat(drive);
};
...
process(left, right) =
  (left, right)
  : par(i, 2, channelStage)
  : _,_ <: par(i, 2, *(1.0 - limitAmt)),
           (co.limiter_lad_stereo(lookahead, limitThreshold, attack, hold, release) : par(i, 2, *(limitAmt)))
      :> _,_
  : softClipPair;
