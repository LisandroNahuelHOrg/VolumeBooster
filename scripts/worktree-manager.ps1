<#
.SYNOPSIS
    SpeedSuite Worktree Multi-Agent Manager v1.0
...
.DESCRIPTION
    ...
    y dist/. La integración a main se hace exclusivamente via CI-validated PRs.
...
function Get-PreferredMainRepo {
    $candidates = @(
        ...
        "D:\OfiDrive\0. Programacion\SpeedSuite Beta"
    )
...
$SpeedCacheRoot = "D:\SpeedSuite-Cache"
...
$GitHubRepo = "LisandroNahuelHOrg/VolumeBooster"
try {
    $remoteUrl = (& git -C $MainRepo config --get remote.origin.url 2>$null).Trim()
    ...
    # fallback al valor legacy si no se puede leer remote
}
$ScopeLockFile = Join-Path $MainRepo ".agent\worktree-scope-lock.json"
$MutexName = "Global\SpeedSuiteScopeLock"
...
$WorktreeManagerDiagDir = Join-Path $MainRepo ".agent\0. Agents Brain\diagnostics\worktree-manager"
$FixedShipAllStateFile = Join-Path $WorktreeManagerDiagDir "fixed-ship-all-active.json"
$FixedShipAllMutexName = "Global\SpeedSuiteFixedShipAllLock"
...
function Wait-MainCIForCommit {
    ...
    Write-Host "⏳ Esperando CI de main para commit $commitShort (PR $prLabel)..."
    Start-Sleep -Seconds 10
    $deadline = (Get-Date).AddSeconds($MaxWaitSeconds)

    while ((Get-Date) -lt $deadline) {
        $runsJson = gh api "repos/$GitHubRepo/actions/workflows/verify.yml/runs?branch=main&per_page=20" 2>&1
        try {
        ...
        Write-Host "   CI main[$($targetRun.id)] commit=${commitShort}: status=$status conclusion=$(if($conclusion){$conclusion}else{'(pendiente)'})"

        if ($status -eq "completed") {
            if ($conclusion -eq "success") {
                Write-Host "✅ CI main del commit ${commitShort}: PASS. Procediendo a limpieza física del worktree."
                return $true
            }

            Write-Host "❌ CI main del commit ${commitShort}: FALLÓ ($conclusion). NO se eliminará el directorio local."
            return $false
        }
    ...
    Write-Host "⏰ Timeout ($MaxWaitSeconds s) esperando CI main para commit $commitShort. Directorio local conservado."
    return $false
}
...
function Wait-MainCI {
    ...
    Write-Host "⏳ Esperando CI de main (100%% verde) antes de limpiar..."
    # Buffer inicial: GitHub tarda ~15s en registrar el merge como nuevo run
    Start-Sleep -Seconds 15
    $deadline = (Get-Date).AddSeconds($MaxWaitSeconds)
    while ((Get-Date) -lt $deadline) {
        $runsJson = gh api "repos/$GitHubRepo/actions/workflows/verify.yml/runs?branch=main&per_page=1" 2>&1
        try {
            $runs = $runsJson | ConvertFrom-Json
        }
        catch {
        ...
        if (-not $latest) {
            Write-Host "   ⚠️ Sin runs en main aún. Reintentando..."
            Start-Sleep -Seconds 20
            continue
        }
        $status = $latest.status
        $conclusion = $latest.conclusion
        Write-Host "   CI main[$($latest.id)]: status=$status conclusion=$(if($conclusion){$conclusion}else{'(pendiente)'})"
        if ($status -eq "completed") {
            if ($conclusion -eq "success") {
                Write-Host "✅ CI main: PASS. Procediendo a limpieza física del worktree."
                return $true
            }
            else {
                Write-Host "❌ CI main: FALLÓ ($conclusion). NO se eliminará el directorio local."
                return $false
            }
        }
    ...
    Write-Host "⏰ Timeout ($MaxWaitSeconds s) esperando CI main. Directorio local conservado."
    return $false
}
    ...
    Use-AtomicLock {
        ...
        if (Test-Path $Config.path) {
            ...
            try {
                ...
                if ($LASTEXITCODE -ne 0) { throw "❌ Error en git fetch origin main --prune" }
                ...
                if ($branchExists) {
                    ...
                    git worktree add $Config.path -b $Config.branch origin/main 2>&1 | Out-Null
                }
                if ($LASTEXITCODE -ne 0) {
...
function Get-MonitorAheadBehind {
    ...
    git -C $Config.path fetch origin main --prune 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw "❌ No se pudo hacer fetch de origin/main para '$($Config.display)'."
    }

    $aheadRaw = (git -C $Config.path rev-list --count "origin/main..$($Config.branch)" | Out-String).Trim()
    $behindRaw = (git -C $Config.path rev-list --count "$($Config.branch)..origin/main" | Out-String).Trim()
...
function Ensure-FixedMonitorUpstreamMain {
    ...
    if ($LASTEXITCODE -eq 0 -and $upstream -eq "origin/main") {
        return
    }

    git -C $Config.path branch --set-upstream-to=origin/main $Config.branch 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "⚠️ No se pudo normalizar upstream a origin/main para '$($Config.display)' durante '$ContextLabel'."
        return
    }

    Write-Host "🔁 Upstream normalizado a origin/main para '$($Config.display)' ($ContextLabel)."
}
...
function Invoke-FixedMonitorOpen {
    ...
    if ($mainSyncOk) {
        Write-Host "✅ main local sincronizado antes de abrir '$($config.display)'."
    }
    else {
        Write-Warning "⚠️ No se pudo sincronizar main local antes de abrir '$($config.display)'. Continuando con estado seguro actual."
    }
    ...
    if (-not $committed -and $aheadBehind.ahead -eq 0 -and $aheadBehind.behind -gt 0) {
        Write-Host "🔄 '$($config.display)' está behind de origin/main sin commits propios. Realineando para iniciar sesión actualizado..."
        git -C $config.path checkout $config.branch 2>&1 | Out-Null
        if ($LASTEXITCODE -ne 0) { throw "❌ No se pudo hacer checkout de '$($config.branch)' durante fixed-open." }

        git -C $config.path reset --hard origin/main 2>&1 | Out-Null
        if ($LASTEXITCODE -ne 0) { throw "❌ No se pudo resetear '$($config.branch)' a origin/main durante fixed-open." }
    ...
    Write-Host "   Estado vs origin/main: ahead=$($aheadBehind.ahead), behind=$($aheadBehind.behind)"
    if ($mainSyncOk) {
        Write-Host "   Main local sync: ✅ actualizado contra origin/main"
    }
    else {
        Write-Host "   Main local sync: ⚠️ no sincronizado (bloqueado por estado local)"
    }
    if ($sessionRealigned) {
        Write-Host "   Session sync monitor: ✅ realineado a origin/main"
    }
    ...
    try {
        ...
        if (-not $dirtyAtStart -and $initialAheadBehind.ahead -eq 0) {
            if ($initialAheadBehind.behind -gt 0) {
                Write-Host "🔄 '$($config.display)' está detrás de origin/main sin cambios propios. Realineando..."
                git checkout $config.branch 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) { throw "❌ No se pudo hacer checkout de '$($config.branch)'." }
                git reset --hard origin/main 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) { throw "❌ No se pudo resetear '$($config.branch)' a origin/main." }
            }
        ...
        Write-Host "🔄 Rebaseando '$($config.branch)' sobre origin/main..."
        ...
        if ([string]::IsNullOrWhiteSpace($mergeCommitSha)) {
            Write-Warning "⚠️ No se pudo resolver merge commit de '$($config.display)'. Fallback CI global de main."
        ...
        if (-not $ciPassed) {
            throw "❌ CI de main no quedó en verde para '$($config.display)'."
        }
        Write-Host "🔄 Realineando rama fija '$($config.branch)' a origin/main..."
        git fetch origin main --prune
        ...
        $mainSyncOk = Sync-MainLocalNonDestructive
        if (-not $mainSyncOk) {
            throw "❌ No se pudo dejar main local sincronizado tras fixed-ship de '$($config.display)'."
        }
...
function Sync-MainLocalNonDestructive {
    ...
    Write-Host "🔄 Sincronizando main local con origin/main (NO destructivo)..."
    Push-Location $MainRepo
    try {
        $dirty = (git status --porcelain | Measure-Object).Count -gt 0
        if ($dirty) {
            Write-Warning "⚠️ Main repo tiene cambios locales. Se omite sync para evitar pérdida accidental."
            return $false
        }
        ...
        if ($currentBranch -ne "main") {
            ...
            if ($LASTEXITCODE -ne 0) {
                Write-Warning "⚠️ No se pudo hacer checkout a main. Se conserva estado local."
                return $false
            }
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "⚠️ git fetch origin main falló. Se conserva estado local."
            return $false
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "⚠️ No fast-forward posible hacia origin/main. Se conserva estado local sin cambios destructivos."
            return $false
        }

        if ($RunBuild) {
            Write-Host "🔨 Compilando build de producción en main local..."
            npm run build
            if ($LASTEXITCODE -ne 0) {
                Write-Warning "⚠️ Build falló. dist/ puede quedar desactualizado."
                return $false
            }
            Write-Host "✅ Build exitoso. dist/ refleja el último estado de main."
        }

        return $true
    }
    catch {
        Write-Warning "⚠️ Error en sincronización no destructiva de main: $_"
        return $false
    }
    finally {
        Pop-Location
...
function Invoke-MainLocalAutosanitizeForShip {
    Write-Host "🧹 Verificando estado de main local antes del ship..."
    ...
    try {
        git fetch origin main --prune
        if ($LASTEXITCODE -ne 0) {
            throw "❌ git fetch origin main --prune falló en main local."
        }
        ...
        try {
            ...
            if ($currentBranch -eq "main") {
                ...
                $abMain = Get-RefAheadBehindCounts -RepoPath $MainRepo -BaseRef "origin/main" -TargetRef "main"

                if (-not $dirtyMain -and $abMain.ahead -eq 0 -and $abMain.behind -eq 0) {
                    Write-Host "✅ main local ya está limpio y sincronizado."
                    return $true
                }

                if (-not $dirtyMain -and $abMain.ahead -eq 0 -and $abMain.behind -gt 0) {
                    Write-Host "🔄 main local está detrás de origin/main sin cambios propios. Fast-forward..."
                    git merge --ff-only origin/main 2>&1 | Out-Null
                    if ($LASTEXITCODE -ne 0) {
                        throw "❌ Fast-forward de main local hacia origin/main falló."
                    }
                    Write-Host "✅ main local fast-forward aplicado."
                    return $true
                }

                $rescueBranch = Get-ValidMainRescueBranchName
                Write-Warning "⚠️ Detectado main local con cambios pendientes (dirty=$dirtyMain, ahead=$($abMain.ahead), behind=$($abMain.behind))."
            ...
            try {
                ...
                if ($dirtyRescue) {
                    ...
                    Write-Host "ℹ️ No había cambios sin commitear; rama de rescate conserva commits locales ahead de main."
                }

                Write-Host "🔄 Rebaseando rama de rescate '$rescueBranch' sobre origin/main..."
                git rebase origin/main -X theirs 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) {
                    git rebase --abort 2>&1 | Out-Null
                    throw "❌ Rebase automático de rama de rescate '$rescueBranch' contra origin/main falló."
                }
                ...
                if (-not $mainCiOk) {
                    throw "❌ CI post-merge de main falló durante saneamiento de main local."
                }

                git fetch origin main --prune
                if ($LASTEXITCODE -ne 0) {
                    throw "❌ Fetch final de main falló tras saneamiento."
                }

                git checkout main 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) {
                    throw "❌ No se pudo volver a 'main' tras saneamiento."
                }

                git reset --hard origin/main 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) {
                    throw "❌ No se pudo resetear main local a origin/main tras saneamiento."
                }

                git branch -D $rescueBranch 2>$null | Out-Null

                $dirtyFinal = (git status --porcelain | Measure-Object).Count -gt 0
                $abFinal = Get-RefAheadBehindCounts -RepoPath $MainRepo -BaseRef "origin/main" -TargetRef "main"
                if ($dirtyFinal -or $abFinal.ahead -ne 0 -or $abFinal.behind -ne 0) {
                    throw "❌ Main local no quedó 100% limpio/sync tras saneamiento (dirty=$dirtyFinal, ahead=$($abFinal.ahead), behind=$($abFinal.behind))."
                }

                Write-Host "✅ Saneamiento de main local completado (dirty=false, ahead=0, behind=0)."
                return $true
            }
            finally {
                if ($lockRegistered) {
...
╔══════════════════════════════════════════════════════════════╗
║           SpeedSuite Worktree Manager v1.0                  ║
╠══════════════════════════════════════════════════════════════╣
║                                                              ║
║  COMMANDS:                                                   ║
...
"@
    ...
    "fixed-ship" {
        ...
        Write-Host "   Post-reset vs origin/main: ahead=$($result.ahead), behind=$($result.behind)"
        Write-Host ""
    }
    ...
    "create" {
        ...
        Use-AtomicLock {
            ...
            try {
                ...
                # Create worktree based on origin/main (NOT local main)
                # This guarantees fresh code even if local main is behind
    ...
    "ship" {
        ...
        try {
            ...
            # Step 3: Rebase on latest remote main
            Write-Host "🔄 Sincronizando con main remoto..."
            git fetch origin main
            ...
            # NOTE: NO git fetch origin main:main — causes fatal error when main is checked out
            # The next 'create' command will fetch origin/main anyway
            ...
            $mergeCommitSha = "$($mergeResult.MergeCommitSha)"
            if ([string]::IsNullOrWhiteSpace($mergeCommitSha)) {
                Write-Warning "⚠️ No se pudo resolver merge commit del PR. Fallback a verificación global de main."
                $ciPassed = Wait-MainCI -MaxWaitSeconds 600
            }
            else {
                $ciPassed = Wait-MainCIForCommit -CommitSha $mergeCommitSha -PrNumber $mergeResult.PrNumber -MaxWaitSeconds 600
            ...
            if ($ciPassed) {
                ...
                # Step 10: Sincronizar main local + build de producción
                ...
                if (-not $syncOk) {
                    throw "❌ Sync/build de main no completado de forma segura tras ship legacy."
                }
                ...
                Write-Host "   Branch $Branch → main (squash-merge)"
                Write-Host "   CI main: ✅ PASS"
                Write-Host "   Main local: $mainLocalStatus"
                ...
                Write-Host ""
                Write-Host "🔄 Sincronizando main local (best-effort NO destructivo)..."
                $syncOk = Sync-MainLocalNonDestructive -RunBuild
                if (-not $syncOk) {
                ...
                Write-Host "   Branch $Branch → main (squash-merge OK)"
                Write-Host "   CI main: no confirmado (fallo o timeout)"
                Write-Host "   Main local: sincronizado (best-effort)"
