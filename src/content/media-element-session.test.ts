...
describe("MediaElementSession", () => {
  ...
  it.each([
    ...
    "warm_cinematic",
    "punch_drive"
  ] as const)("routes the %s sound preset into the Faust runtime exactly", async (preset) => {
  ...
  it.each([
    ...
    "treble_safe",
    "punch_preserve"
  ] as const)("routes the %s protector profile into the Faust runtime exactly", async (mode) => {
    const settings = {
      ...DEFAULT_ADVANCED_AUDIO_SETTINGS,
    ...
    expect(faustNode.paramValues.get("lowBandThresholdOffsetDb")).toBe(
      expectedRuntime.lowBandThresholdOffsetDb
    );
    expect(faustNode.paramValues.get("lowBandRatioBias")).toBe(expectedRuntime.lowBandRatioBias);
    expect(faustNode.paramValues.get("midHighThresholdOffsetDb")).toBe(
      expectedRuntime.midHighThresholdOffsetDb
    );
    expect(faustNode.paramValues.get("outputCeilingDb")).toBe(expectedRuntime.outputCeilingDb);
    expect(faustNode.paramValues.get("outputSoftClipMix")).toBe(expectedRuntime.outputSoftClipMix);
    expect(faustNode.paramValues.get("clarityPresenceTiltDb")).toBe(
      expectedRuntime.clarityPresenceTiltDb
    );
  });
