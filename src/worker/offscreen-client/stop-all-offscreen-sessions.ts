...
export async function stopAllOffscreenSessions(): Promise<CaptureSessionState[]> {
  if (!(await hasOffscreenDocument())) {
    return [];
  }

  const response = await sendOffscreenMessage<SnapshotResponse>({ type: "OFFSCREEN_STOP_ALL" });
  return resolveOffscreenSnapshotResponse(response, "errorOffscreenStopAll");
}
